...
class ComputeShaderExample(mglw.WindowConfig):
    ...
    def __init__(self, **kwargs) -> None:
        ...
        projection_matrix = perspective(60, self.wnd.aspect_ratio, 0.1, 1000).astype(
            "f4"
        )
        ...
        _rgb_colors = np.array(
            (np.arange(self.N) / self.N, np.full(self.N, 0.7), np.full(self.N, 0.5))
        ).T
        # convert to grb
    ...
    def render(self, time: float, frame_time: float) -> None:
        ...
        rotation = rotate((0, 0, time / 3.0), dtype="f4")
    ...
    def generate_data(self) -> np.array:
        ...
        positions = (np.random.random((self.N, 3)) - 0.5) * self.BOX_SIZE
        velocities = np.random.random((self.N, 3)) - 0.5
        velocities *= self.SPEED
        return np.column_stack((positions, velocities)).flatten().astype("f4")


if __name__ == "__main__":
    ComputeShaderExample.run()
