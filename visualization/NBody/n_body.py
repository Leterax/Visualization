...
sun = {
    "location": (0, 0, 0),
    "mass": 1.989e30 / 1.989e30,
    "velocity": (0, 0, 0),
    "force": (0, 0, 0),
}
mercury = {
    "location": (0, 5.7e10 / 1.5e11, 0),
    "mass": 3.285e23 / 1.989e30,
    "velocity": (47e3 / 1.5e11, 0, 0),
    "force": (0, 0, 0),
}
venus = {
    "location": (0, 1.1e11 / 1.5e11, 0),
    "mass": 4.8e24 / 1.989e30,
    "velocity": (35e3 / 1.5e11, 0, 0),
    "force": (0, 0, 0),
}
earth = {
    "location": (0, 1.5e11 / 1.5e11, 0),
    "mass": 6e24 / 1.989e30,
    "velocity": (30e3 / 1.5e11, 0, 0),
    "force": (0, 0, 0),
}
mars = {
    "location": (0, 2.2e11 / 1.5e11, 0),
    "mass": 2.4e24 / 1.989e30,
    "velocity": (24e3 / 1.5e11, 0, 0),
    "force": (0, 0, 0),
}
jupiter = {
    "location": (0, 7.7e11 / 1.5e11, 0),
    "mass": 1e28 / 1.989e30,
    "velocity": (13e3 / 1.5e11, 0, 0),
    "force": (0, 0, 0),
}
saturn = {
    "location": (0, 1.4e12 / 1.5e11, 0),
    "mass": 5.7e26 / 1.989e30,
    "velocity": (9e3 / 1.5e11, 0, 0),
    "force": (0, 0, 0),
}
uranus = {
    "location": (0, 2.8e12 / 1.5e11, 0),
    "mass": 8.7e25 / 1.989e30,
    "velocity": (6.835e3 / 1.5e11, 0, 0),
    "force": (0, 0, 0),
}
neptune = {
    "location": (0, 4.5e12 / 1.5e11, 0),
    "mass": 1e26 / 1.989e30,
    "velocity": (5.477e3 / 1.5e11, 0, 0),
    "force": (0, 0, 0),
}
...
class NBodySim(mglw.WindowConfig):
    ...
    def __init__(self, **kwargs):
        ...
        # load programs
        self.calculate_force = self.load_compute_shader(
            "calculate_force.glsl", defines=self.consts
        )
        self.move_planets = self.load_compute_shader(
            "move_planets.glsl", defines=self.consts
        )

        self.render_program = self.load_program("points.glsl")

        # set projection matrices
        projection_matrix = perspective(60, self.wnd.aspect_ratio, 0.001, 7e15).astype(
            "f4"
        )
        ...
        # interleave data
        interleaved = list(
            zip(positions.tolist(), velocities.tolist(), force.tolist(), mass.tolist())
        )
    ...
    def render(self, time, frame_time):
        ...
        if (
            ...
            and self.simulated_time - self._last_capture > self.gif_interval
        ):
            # take a screenshot
